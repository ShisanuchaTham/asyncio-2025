...
async def add_student(student: Student):
    try:
        async with aiosqlite.connect(DATABASE) as db:
            await db.execute("INSERT INTO students (student_id, name, enrolled_year, group_no) VALUES (?, ?, ?, ?)", 
...
async def update_student(student_id: str, student: Student):
    async with aiosqlite.connect(DATABASE) as db:
        cursor = await db.execute("UPDATE students SET name = ?, enrolled_year = ?, group_no = ? WHERE student_id = ?", 
...
async def analytics_group():
    async with aiosqlite.connect(DATABASE) as db:
        cursor = await db.execute(
            "SELECT grou    p_no, COUNT(*) as student_count FROM students GROUP BY group_no")
